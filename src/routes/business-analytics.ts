...
function getPeriodRange(
  ...
  if (period === "year") return { startDate: `${y}-01-01`, endDate: `${y}-12-31` };
  if (period === "quarter") {
...
async function getVisits(startDate: string, endDate: string) {
 ...
 */
const REAL_VISIT_FILTER = (v: any) =>
  !v.is_cycle_contract && (!v.is_cycle || v.status === "completed");

/* ──────────────────────────────────────────
   FORECAST UTILITIES (fix Bug 4: corrected CI formula)
   ────────────────────────────────────────── */

function buildMonthlySeries(visits: any[]) {
...
function computeSeasonalityWeighted(
 ...
 */
function linearForecast(series: { month: string; revenue: number }[]) {
  if (series.length < 2) {
...
function computeConfInterval(
  ...
  const se =
    stdDev * Math.sqrt(1 + 1 / n + Math.pow(xNew - meanX, 2) / (sumSqD || 1));
  return 1.96 * se; // z-score per 95% CI
}

function buildMonthlySeriesByKey(
  visits: any[],
  keyField: string,
  valueField: string,
) {
...
function computeStrengths(
  byKey: Record<string, { revenue: number; visits: number; margin: number }>,
) {
...
async function getCurrentCompensation(
 ...
 */
async function loadAllCompensations(): Promise<Map<string, number>> {
  const snapshot = await db
    .collection("professional_compensations")
    .where("effective_to", "==", null)
    .get();
 ...
 */
function parseSessionProfessionals(
  raw: any,
  nSessions: number,
  defaultProfessional: string,
): Array<{ session_number: number; professional_name: string }> {
  const result: Array<{ session_number: number; professional_name: string }> = [];
  const rawList = raw?.sedute_professionisti || raw?.session_professionals;
...
export function setupBusinessAnalyticsRoutes() {
  ...
  router.post("/import", async (req: any, res: any) => {
    try {
      ...
      const compensationMap = await loadAllCompensations();

      const batchSize = 400;
      let batches = [db.batch()];
      let batchIndex = 0;
      let opCount = 0;
      const addToBatch = (ref: any, docData: any) => {
      ...
      for (const row of data) {
        ...
        if (!row.Professionista && !row.professionista) continue;
        if (!row.Paziente && !row.paziente) continue;
        if (!date) continue;
        ...
        if (!pct || pct === 0) {
          ...
          if (configPct !== undefined && configPct !== null) pct = configPct;
        }

        const professionalFee = amount > 0 ? Math.round(amount * pct) / 100 : 0;
        const profit =
          parseNumber(row.Utile || row.utile) ||
          Math.round((amount - professionalFee) * 100) / 100;
        const unitFee =
          parseNumber(row.spettanza_unit || row.Spettanza_unit) ||
          (nSessions > 0 ? Math.round((professionalFee / nSessions) * 100) / 100 : 0);
        const unitProfit =
          parseNumber(row.Utile_unit || row.utile_unit) ||
        ...
        if (isCycle) {
          ...
          addToBatch(db.collection(COLLECTION).doc(), {
          ...
          for (let s = 1; s <= nSessions; s++) {
            ...
            const sp = sessionProfessionals.find(
              (x) => x.session_number === s,
            );
            const sessionProfessional = sp?.professional_name || professional;
          ...
          addToBatch(db.collection(CYCLES_COLLECTION).doc(cycleId), {
            patient_name: patient,
            professional_name: professional,
            service_name: service,
            branch,
  ...
  /* ==========================================
     ...
     ========================================== */
  router.post("/manual-entry", async (req: any, res: any) => {
    try {
      const {
        patient_name,
        professional_name,
      ...
      if (sessions > 1) {
        ...
        // Multi-prof: parsing session_professionals
        const sessionProfs = parseSessionProfessionals(
          { session_professionals },
          sessions,
          professional_name,
        );
        ...
        // Contratto
        const contractRef = db.collection(COLLECTION).doc();
        const contractData = {
          patient_name,
          professional_name,
        ...
        created.push({ id: contractRef.id, ...contractData });
        ...
        for (let s = 1; s <= sessions; s++) {
          ...
          const sessionProfessional = sp?.professional_name || professional_name;
          ...
          created.push({ id: ref.id, ...sessionData });
        }

        // Record ciclo con metadati multi-prof
        batch.set(db.collection(CYCLES_COLLECTION).doc(cycleId), {
          patient_name,
          professional_name,
          service_name: service_name || "",
          branch: branch || "Altro",
        ...
        created.push({ id: ref.id, ...visitData });
      }
  ...
  /* ==========================================
     ...
     ========================================== */
  router.post("/cycle", async (req: any, res: any) => {
    try {
      const {
        patient_name,
        professional_name,
  ...
  /* ==========================================
     ...
     ========================================== */
  router.put("/cycle/:id/session", async (req: any, res: any) => {
    try {
      const { id } = req.params;
  ...
  /* ==========================================
     ...
     ========================================== */
  router.get("/revenue-trend", async (req: any, res: any) => {
    try {
      const { year, compareYoY } = req.query;
      const y = year ? parseInt(year, 10) : new Date().getFullYear();
      const startDate = `${y}-01-01`;
      const endDate = `${y}-12-31`;
      const visits = await getVisits(startDate, endDate);
      ...
      visits.forEach((v) => {
        ...
        if (v.is_cycle_contract) {
          ...
          if (v.patient_name) monthly[key].patients.add(v.patient_name);
          if (!v.is_cycle) {
      ...
      let prevYearData: any[] | null = null;
      if (compareYoY === "true" || compareYoY === "1") {
        const prevStart = `${y - 1}-01-01`;
        const prevEnd = `${y - 1}-12-31`;
        const prevVisits = await getVisits(prevStart, prevEnd);
  ...
  /* ==========================================
     ...
     ========================================== */
  router.get("/by-branch", async (req: any, res: any) => {
    try {
      ...
      realVisits.forEach((v) => {
        ...
        if (v.is_cycle) byBranch[branch].cycles += 1;
        else byBranch[branch].singles += 1;
      });
  ...
  /* ==========================================
     ...
     ========================================== */
  router.get("/patient-flow", async (req: any, res: any) => {
    try {
      const { period = "year", year, month, quarter } = req.query;
      const { startDate, endDate } = getPeriodRange(period, year, month, quarter);
      const visits = await getVisits(startDate, endDate);
      ...
      const allVisitsSnap = await db
        ...
        .orderBy("date", "asc")
        .get();
      const allVisits = allVisitsSnap.docs.map((doc) => doc.data()) as any[];
      ...
      Object.entries(patientVisits).forEach(([, count]) => {
        if (count === 1) freqDistribution["1"]++;
        else if (count <= 3) freqDistribution["2-3"]++;
        else if (count <= 6) freqDistribution["4-6"]++;
        else if (count <= 10) freqDistribution["7-10"]++;
        else freqDistribution["10+"]++;
      });

      Object.values(patientBranches).forEach((branches) => {
        if (branches.size >= 2) multiServiceCount++;
      });
      ...
      const monthlyFlow: Record<string, { newPx: number; returning: number }> = {};
      const start = new Date(startDate);
      const end = new Date(endDate);
  ...
  /* ==========================================
     ...
     ========================================== */
  router.get("/cycle-analysis", async (req: any, res: any) => {
    try {
      ...
      if (branch) contracts = contracts.filter((v) => v.branch === branch);
      // Multi-prof: filtra anche se il professional è tra i professionals del ciclo
      if (professional) {
        const matchingCycleIds = new Set<string>();
        ...
        contracts = contracts.filter(
          (v) => v.professional_name === professional || matchingCycleIds.has(v.cycle_id),
        );
      }
      ...
      const cyclesSnap = await db
        ...
        .get();
      let cycles = cyclesSnap.docs.map((d) => ({ id: d.id, ...d.data() })) as any[];
      ...
      if (professional) {
        const profCycles = cycles.filter(
          (c) =>
      ...
      const completionRate =
        totalCycles > 0 ? Math.round((completedCycles / totalCycles) * 1000) / 10 : 0;
      ...
      const completedWithDuration = cycles
        ...
        .map((c) => {
          ...
          return { ...c, durationDays: Math.max(0, days) };
        })
        .filter((c) => c.durationDays > 0);
      const avgDurationDays =
        completedWithDuration.length > 0
          ? Math.round(
              completedWithDuration.reduce((sum, c) => sum + c.durationDays, 0) /
      ...
      const realizedVisits = visits.filter(
        (v) => v.is_cycle && !v.is_cycle_contract && v.status === "completed",
      );
      ...
      completedWithDuration.forEach((c) => {
        ...
        if (days <= 30) durationBuckets["0-30"]++;
        else if (days <= 60) durationBuckets["31-60"]++;
        else if (days <= 90) durationBuckets["61-90"]++;
        else if (days <= 120) durationBuckets["91-120"]++;
        else durationBuckets["120+"]++;
      });

      // Trend mensile cicli venduti vs completati
      const monthlyCycles: Record<string, { sold: number; completed: number }> = {};
      for (let m = 1; m <= 12; m++) {
      ...
      contracts.forEach((v) => {
        ...
        if (monthlyCycles[key]) monthlyCycles[key].sold += 1;
      });
      cycles.forEach((c) => {
        if (c.status === "completed" && c.end_date) {
          const key = c.end_date.slice(0, 7);
          if (monthlyCycles[key]) monthlyCycles[key].completed += 1;
        }
      });
      ...
      const avgSessions =
        totalCycles > 0
          ? Math.round(
              (cycles.reduce((s, c) => s + (c.total_sessions || 0), 0) / totalCycles) * 10,
            ) / 10
          : 0;

      // Cicli per branca
      const byBranch: Record<string, { cycles: number; contractValue: number }> = {};
      cycles.forEach((c) => {
        const b = c.branch || "Altro";
        if (!byBranch[b]) byBranch[b] = { cycles: 0, contractValue: 0 };
        byBranch[b].cycles += 1;
      ...
      res.json({
        ...
        byBranch: Object.entries(byBranch)
          .map(([name, data]) => ({
            name,
            ...data,
            contractValue: Math.round(data.contractValue * 100) / 100,
          }))
  ...
  router.get("/overview", async (req: any, res: any) => {
    try {
      ...
      let deltaPatients: number | null = null;
      let deltaTicket: number | null = null;

      try {
        const prevVisits = await getVisits(prev.startDate, prev.endDate);
      ...
      const byService: Record<string, { revenue: number; visits: number }> = {};
      const byProfessional: Record<string, { revenue: number; visits: number; margin: number }> = {};

      realVisits.forEach((v) => {
        const service = v.service_name || "Altro";
        if (!byService[service]) byService[service] = { revenue: 0, visits: 0 };
        byService[service].revenue += v.amount || 0;
        byService[service].visits += 1;

        const prof = v.professional_name || "N/A";
        if (!byProfessional[prof]) byProfessional[prof] = { revenue: 0, visits: 0, margin: 0 };
  ...
  /* ==========================================
     ...
     ========================================== */
  router.get("/mix", async (req: any, res: any) => {
    try {
      const { period = "month", year, month, quarter } = req.query;
      const { startDate, endDate } = getPeriodRange(period, year, month, quarter);
      const visits = await getVisits(startDate, endDate);
      ...
      const byService: Record<string, { revenue: number; visits: number }> = {};
      const byBranch: Record<string, { revenue: number; visits: number }> = {};
      const byProfessional: Record<string, { revenue: number; visits: number }> = {};
      ...
      realVisits.forEach((v) => {
        ...
        if (!byService[service]) byService[service] = { revenue: 0, visits: 0 };
        if (!byBranch[branch]) byBranch[branch] = { revenue: 0, visits: 0 };
        if (!byProfessional[prof]) byProfessional[prof] = { revenue: 0, visits: 0 };
      ...
      const normalize = (data: Record<string, { revenue: number; visits: number }>) =>
        Object.entries(data)
          .map(([name, v]) => ({
            name,
  ...
  router.get("/performance", async (req: any, res: any) => {
    try {
      ...
      const realVisits = visits.filter(
        (v) => !v.is_cycle_contract && (!v.is_cycle || v.status === "completed"),
      );
      ...
      realVisits.forEach((v) => {
        ...
        byProfessional[prof].visits += 1;
        byProfessional[prof].margin += v.clinic_revenue || 0;
        if (v.patient_name) byProfessional[prof].patients.add(v.patient_name);
      });
  ...
  /* ==========================================
     ...
     ========================================== */
  router.get("/retention", async (req: any, res: any) => {
    try {
      const { period = "month", year, month, quarter } = req.query;
      const { startDate, endDate } = getPeriodRange(period, year, month, quarter);
      const visits = await getVisits(startDate, endDate);
      ...
      const byPatient: Record<string, { visits: number; revenue: number }> = {};
      realVisits.forEach((v) => {
        const name = v.patient_name || "N/A";
        if (!byPatient[name]) byPatient[name] = { visits: 0, revenue: 0 };
      ...
      const avgVisits =
        totalPatients > 0 ? Math.round((realVisits.length / totalPatients) * 10) / 10 : 0;
      const totalRevenue = realVisits.reduce((s, v) => s + (v.amount || 0), 0);
      const avgRevenuePerPatient =
        totalPatients > 0 ? Math.round((totalRevenue / totalPatients) * 100) / 100 : 0;
      ...
      res.json({
        ...
        returnRate:
          totalPatients > 0 ? Math.round((returning / totalPatients) * 1000) / 10 : 0,
        avgVisits,
  ...
  router.get("/forecast", async (req: any, res: any) => {
    try {
      ...
      let visits: any[] = await getAllVisits();
